...
# Set up guide:
After cloning 
run clang++ -std=c++17 -O2 *.cpp -o nim
...
2. Source code does not create global variables.
...
To receive *Proficient* in the **Applied Programming** learning objective,
your project must satisfy **all** of the following requirements:

1. Your finished project must compile on buffy.
...
2. Process command line arguments: 
   ...
   For example: `take-away -p 3` should automatically start a game with 3 piles.
   If using `-p 3 -s 5`, the the game should put exactly 5 stones in 3 piles.

3. Prompt for the names of two players.
...
4. If the `-p` or `-s` switches are set, 
   do not allow either parameter to be set < 1.
   You may choose to define a larger minimum value.

   You may also set a maximum value.

   If these switches are not set, then their values **must be determined randomly**.
   The same constraints used for manual entry apply to randomly generated values.

5.  All project 'basic gameplay' is implemented correctly.

6.  While the program executes, it should be clear:

    a. Whose turn it is  
    b. What piles can be selected  
    c. How many stones are in each pile  

7. Program definitions and declarations must be kept in separate source files.
